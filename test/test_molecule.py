...
class TestMolecule(unittest.TestCase):
    ...
    def test_molecule_is_super_molecule(self):
        ...
        assert Molecule(NaCl).is_super_molecule(Molecule([Na, Cl])) is True
        assert Molecule(NaCl).is_super_molecule(Molecule([Na2, Cl])) is False
        assert Molecule(H2O).is_super_molecule(Molecule(H2O2)) is False
        assert Molecule(H2O2).is_super_molecule(Molecule(H2O)) is True
